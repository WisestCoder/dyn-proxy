...
class ProxyCore extends events.EventEmitter {
  ...
  constructor(config) {
    ...
    global.recorder = config.recorder;

    if (parseInt(process.versions.node.split('.')[0], 10) < 4) {
      throw new Error('node.js >= v4.x is required for dynproxy');
    } else if (config.forceProxyHttps && !certMgr.ifRootCAFileExists()) {
      logUtil.printLog('You can run `dynproxy-ca` to generate one root CA and then re-run this command');
      throw new Error('root CA not found. Please run `dynproxy-ca` to generate one first.');
    ...
    }, this.proxyRule, global.recorder);
  }
...
class ProxyServer extends ProxyCore {
  ...
  start() {
    if (global.recorder) {
      global.recorder.setDbAutoCompact();
    }

    // start web interface if neeeded
    if (this.proxyWebinterfaceConfig && this.proxyWebinterfaceConfig.enable) {
      this.webServerInstance = new WebInterface(this.proxyWebinterfaceConfig, global.recorder);
      // start web server
  ...
  close() {
    ...
    // release recorder
    if (global.recorder) {
      global.recorder.stopDbAutoCompact();
